...
export function Logo({ size = 'md', iconOnly = false, onClick }: LogoProps) {
  const sizes = {
    ...
    lg: { icon: 'w-14 h-14', text: 'text-2xl', iconText: 'text-base' },
  };
  ...
  return (
    ...
    >
      ...
      <div className={`relative ${iconSize} rounded-2xl bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-700 shadow-lg flex items-center justify-center overflow-hidden group`}>
        ...
        {/* Текст T24 */}
        <span className={`relative z-10 text-white ${iconText} font-semibold tracking-tight drop-shadow-md`}>
          T24
        </span>
      ...
      {!iconOnly && (
        <div className="flex flex-col -space-y-1">
          <span className={`bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent font-semibold tracking-tight ${textSize}`}>
            T24
          </span>
...
export function LogoIcon({ size = 'md' }: { size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <div className={`relative ${wrapper} rounded-xl bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-700 shadow-lg flex items-center justify-center overflow-hidden group`}>
      ...
      {/* Текст T24 */}
      <span className={`relative z-10 text-white ${text} font-semibold tracking-tight drop-shadow-md`}>
        T24
      </span>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <motion.span
          className="relative z-10 text-white text-2xl font-bold tracking-tight drop-shadow-lg"
          animate={{
            textShadow: [
      ...
      <motion.div
        ...
        className="flex flex-col items-center"
      >
        <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent mb-1 text-[36px]">
          T24
        </h1>
      </motion.div>
    </motion.div>
