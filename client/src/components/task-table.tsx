...
import { ArrowUpDown, ArrowUp, ArrowDown, Flame, Tag, User } from 'lucide-react';
...
export function TaskTable({ searchQuery, filters, onTaskClick }: TaskTableProps) {
  const { tasks, projects, teamMembers, currentUser, customColumns, updateTask } = useApp();
      ...
      result.sort((a, b) => {
        ...
        switch (sortColumn) {
          ...
          case 'project':
            ...
            break;
          case 'category':
            const aCategory = mockCategories.find(c => c.id === a.categoryId);
            const bCategory = mockCategories.find(c => c.id === b.categoryId);
            aVal = aCategory?.name || '';
            bVal = bCategory?.name || '';
            break;
          case 'status':
